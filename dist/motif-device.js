...
`);
  ...
  function emitContext() {
    outlet(
      1,
      "context",
      `${noteName(hostContext.rootNote)} ${hostContext.scaleName}`,
      `${hostContext.timeSignature.numerator}/${hostContext.timeSignature.denominator}`,
      Math.round(hostContext.tempo * 100) / 100,
      hostContext.isPlaying ? "Playing" : "Stopped"
    );
  }
  ...
  function observeNumber(property, apply) {
    ...
    if (!api.valid) {
      emitError(`Unable to observe Live property: ${property}`);
      return;
    }
  ...
  function observeString(property, apply) {
    const api = new LiveAPI((args) => {
      const value = callbackValues(args).map(String).join(" ").trim();
      if (value) {
        apply(value);
        emitContext();
      }
  ...
  function initialize() {
    ...
    observeNumber("is_playing", (value) => {
