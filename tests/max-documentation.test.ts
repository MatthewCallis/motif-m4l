...
describe('Max documentation contract', () => {
  it('links every generated Max object class to current official documentation', async () => {
    ...
    for (const box of collectBoxes(patch.patcher.boxes)) {
      const surface = box.maxclass === 'newobj'
        ? String(box.text ?? '').trim().split(/\s+/, 1)[0]
        : box.maxclass;
    ...
    for (const surface of surfaces) {
      ...
      assert.ok(documentation.includes(url), `${surface} documentation link is missing from MAX-DOCUMENTATION.md`);
    }
  });
