...
describe('compileMotif', () => {
  ...
  it('applies velocity curves, note overrides, scaling, offsets, and clamping', () => {
    ...
    assert.equal(applyVelocityCurve(1, { inputMin: 1, inputMax: 127, outputMin: 10, outputMax: 110 }), 10);
    assert.equal(applyVelocityCurve(127, { inputMin: 1, inputMax: 127, outputMin: 10, outputMax: 110 }), 110);
  ...
  it('extends durations for legato and matching ties', () => {
    ...
    assert.equal(effectiveDuration({ at: 0, duration: 120, pitch: 0, legato: true }, next, MOTIF), 480);
    assert.equal(effectiveDuration({ at: 0, duration: 600, pitch: 0, tie: true }, next, MOTIF), 960);
    assert.equal(effectiveDuration({ at: 0, duration: 600, pitch: 1, tie: true }, next, MOTIF), 600);
    assert.equal(effectiveDuration({ at: 0, duration: 100, pitch: 0, gate: 0 }, undefined, MOTIF), 1);
  });
  ...
  it('skips sparse notes and sorts note-offs before simultaneous note-ons', () => {
    ...
    assert.deepEqual(events.map(({ offsetTicks, velocity }) => [offsetTicks, velocity]), [
      [0, 100],
      [10, 0],
      [10, 100],
      [20, 0],
