...
function event(instanceId: number, velocity: number, offsetTicks: number): ScheduledMidiEvent {
  return {
    pitch: 60,
    velocity,
    channel: 1,
...
describe("RuntimeScheduler", () => {
  it("does not emit overlapping duplicate note-ons or premature note-offs", () => {
    ...
    assert.deepEqual(
      first.map(({ velocity, delay }) => [velocity, delay]),
      [
        [100, 0],
        [0, 960],
      ],
    ...
    assert.deepEqual(
      second.map(({ velocity, delay }) => [velocity, delay]),
      [[0, 960]],
    );
  });
  ...
  it("cancels multiple instances in one queue rebuild", () => {
    ...
    assert.deepEqual(
      output.filter(({ velocity }) => velocity === 0).map(({ pitch }) => pitch),
      [60],
    );
  });
  ...
  it("cancels one active instance and ignores missing or inactive holds", () => {
    ...
    assert.deepEqual(
      cancelled.map(({ velocity, instanceId }) => [velocity, instanceId]),
      [[0, 3]],
    );
