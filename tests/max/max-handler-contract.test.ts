...
const PATCH_MESSAGES: ReadonlyArray<readonly [string, ...unknown[]]> = [
  ...
  ["pitch_mode", "motif"],
  ["scale_override_root", 0],
  ["scale_override_name", "Major"],
  ["scale_override", 1],
  ["scale_override", 0],
  ["invert", 1],
  ["invert", 0],
  ...
  [
    "lib_action",
    encodeURIComponent(
      JSON.stringify({ type: "edit_note_at", index: 0, field: "legato", value: true }),
    ),
  ],
  [
    "lib_action",
    encodeURIComponent(
      JSON.stringify({ type: "map_trigger", pitch: 36, motifId: "chromatic-turn" }),
    ),
  ],
  [
    "lib_action",
