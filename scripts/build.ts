...
const libraryStyle = (
  await transform(libraryStyleSource, {
    loader: "css",
    target: "chrome80",
    minify: true,
    sourcemap: false,
...
const productionInputs = new Set(
  [...Object.keys(buildResult.metafile.inputs), ...Object.keys(libraryBuild.metafile.inputs)].map(
    (input) => path.normalize(input),
  ),
);
const unusedSourceModules = (await listTypeScriptModules("src")).filter(
  (modulePath) => !productionInputs.has(path.normalize(modulePath)),
);
